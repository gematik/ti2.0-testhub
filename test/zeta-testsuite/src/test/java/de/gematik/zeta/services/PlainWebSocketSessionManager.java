...
public class PlainWebSocketSessionManager {
  ...
  @Setter
  private static int connectionTimeoutSeconds = resolveInt("zeta.timeouts.wsConnectionSeconds");

  @Setter private static int messageTimeoutSeconds = resolveInt("zeta.timeouts.wsMessageSeconds");

  @Setter @Getter private static String proxyUri = null;

  private static int resolveInt(String key) {
    return Integer.parseInt(TigerGlobalConfiguration.resolvePlaceholders("${" + key + "}"));
  }
