...
To simply switch between different configuration for testing purposes one can apply additional `.env` files with the
parameter `--env_file` when starting docker compose setup. Each `.env` file is defined with its own parameter:
```shell
docker compose -f ./doc/docker/compose-local.yaml --env-file=./doc/docker/.env --env-file=./doc/docker/env-private/.my-own.env --profile ps-only
```

The table below contains a list of currently used parameters used to overwrite corresponding variables in the docker
compose YAMLs.
...
Two examples demonstrating the usage of a real PoPP service or the PoPP token generator can be found here:

* `doc/docker/.dev-with-popp-service-rise.env`
* `doc/docker/.dev-with-popp-token-generator.env`
