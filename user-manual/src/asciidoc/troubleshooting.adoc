...
This happens when you are using the Tiger Workflow UI and you didn't press the power off button in the Workflow UI. To fix the issues:
...
NOTE: We can not provide support for using Podman.
The following are just some hints how to adapt the build process to use Podman instead of Docker.
Please refer to the Podman documentation for further details.

The Maven profile `-Pdocker` calls the `docker` CLI directly (`docker build ...`).
If Docker is not installed, you can apply a local (not committed) POM override to use Podman.

==== Use `-Pdocker` with Podman image storage
...
2. Add `container.cli` in the root POM `pom.xml` (local change only):
...
3. Replace `executable="docker"` with `executable="${container.cli}"` in:

* `pom.xml`
* `server/vsdm-server-simservice/pom.xml`

4. Run the build with Podman:

[source,bash]
----
./mvnw clean install -Pdocker -DskipTests -Dcontainer.cli=podman
----

5. Verify that the created image is available in Podman:
...
==== Docker commands can be executed with Podman

Most commands from this documentation can be mapped 1:1 from Docker to Podman.
...
# Podman equivalent
podman compose -f ./doc/docker/compose-local.yaml --profile full up -d --remove-orphans
----

If `podman compose` is not available in your setup, use `podman-compose` instead:
