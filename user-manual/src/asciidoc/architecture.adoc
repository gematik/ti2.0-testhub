...
| **Card Terminal Client** | 8000 | Simulates electronic health card (eGK, SMC-B, HBA) terminal operations with PACE protocol support
| **VSDM Client**          | 8220 | Simulates VSDM2 client operations including data retrieval and PoPP token validation
| **ZeTA PEP PoPP**        | 9110 | Policy Enforcement Point proxy for PoPP validates tokens and forwards requests
| **ZeTA PDP PoPP**        | 9112 | Policy Decision Point for PoPP performs OAuth 2.0 token exchange (RFC 8693)
| **PoPP Server**          | 9210 | Backend PoPP service generates and validates Proof of Possession tokens
| **VSDM Server**          | 9130 | Backend VSDM service provides VSDM2 data from YAML test fixtures
